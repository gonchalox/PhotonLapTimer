...
#include "Particle.h"
#line 1 "/Users/gonzalo/Documents/particle/PhotonLapTimer/src/PhotonLapTimer.ino"
/*
 * Project PhotonLapTimer
 * Description: A simple lap timer for Particle Photon
 * Author: Gonzalo Luzardo  
 * Date: March 2020
 */

#include "PhotonLapTimer.h"
#include "kalman.h"
#include <math.h>
#include "const.h"
#include "kalman.h"
...
#line 16 "/Users/gonzalo/Documents/particle/PhotonLapTimer/src/PhotonLapTimer.ino"
KalmanFilter kalmanFilter;

uint8_t deviceID = 0;
deviceState_e state;
lapTimer_t lapTimer;
RX5808 myRX5808(PIN_RSSI, PIN_SPI_DATA, PIN_SLAVE_SELECT, PIN_SPI_CLOCK);
...
void calibrateRSSI(void)
{
  ...
  for (uint8_t i = 0; i < RSSI_THRESHOLD_CALIBRATE_READS; i++) {
    rssi_average += round(kalmanFilter.filter(myRX5808.readRssi(), 0));
  }
...
void blink(uint16_t time){
  #ifdef BUZZER
    digitalWrite(PIN_LED, LOW); // activate beep
    delay(time / 2);
    digitalWrite(PIN_LED, HIGH);
  #endif
}
